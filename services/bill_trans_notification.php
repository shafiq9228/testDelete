...
$notif = "Payment successfull to  $ser[service_name] service with Rs. $amount  on ".date('d-m-Y h:i',strtotime($datetime))." ";


$info =  $db->query("SELECT * FROM `register` where guid = '$userid' ")->fetch();
  

$message = "Dear Customer - Payment for $ser[service_name] is successfull with amount Rs.$amount.Your transaction ID is $val, your balance credit is $balance as on $today.";
$sms=str_replace(" ","%20","$message"); 


 

$url = "https://49.50.67.32/smsapi/httpapi.jsp?username=dakshin&password=54321&from=DAKNPY&to=$info[mobile]&text=$sms&coding=0&pe_id=1001724022979357639&template_id=1007447995165063924";
...
$template_id = '1007447995165063924';
$from = 'DAKNPY';
$sms_text = urlencode("Dear Customer - Payment for $ser[service_name] is successfull with amount Rs.$amount.Your transaction ID is $val, your balance credit is $balance as on $today.");
...
curl_close($ch);
    ...
    $singleID = $info['regid']; 
    $fcmMsg = array(
    	'body' => "Payment successful with Rs.$amount for $ser[service_name]",
    	'title' => "Order Status",
    	'sound' => "default",
        'color' => "#203E78" 
    );
...
include "templates/bill_trans_email.php"
   ...
   background: #032553;  border-radius: 10px; ">
      ...
      <tr >
        <td align="center" style="padding-bottom: 0px;"><p class="due">
          ...
          Has been debited from your Dakshin Pay Account ('.$reg[mobile].') for '.$ser[service_name].' service with '.$number.' . Your transaction ID is #'.$val.'. </p></td>
      </tr>
