...
$postad = $db->query("INSERT INTO transactions (`type`,`pincode`,`transid`,`userid`,`merchant`,`merchantname`,`logo`,`paidby`,`customerno`,`amount`,`message`,`status`,`date`,`datetime`,`bill_status`,`sound_notification`,`service`,`provider`,`number`,`client_id`,`provider_id`,`validity`,`status_message`) VALUES ('Services','$reg[pincode]','$val','$userid','0','$pro[provider_name]','$pro[icon]','$reg[name]','$reg[mobile]','$amount','success','Pending','$date','$datetime','0','0','$ser[service_name]','$pro[provider_name]','$number','$client_id','$provider_id','$validity','Pending')");
...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://api.pay2all.in/v1/payment/recharge?optional1='.$optional1,
  ...
  CURLOPT_POSTFIELDS => 'number='.$number.'&amount='.$amount.'&provider_id='.$provider_id.'&client_id='.$client_id.'&api_id='.$api_id.'&reference_id='.$reference_id.'&payment_mode='.$payment_mode,
...
$sql = $db->query("INSERT INTO recharges (`pincode`,`transid`,`userid`,`customerno`,`amount`,`message`,`status`,`date`,`datetime`,`service`,`provider_name`,`provider_id`,`utr`,`reportid`,`orderid`,`status_id`,`number`,`client_id`,`expiry_date`,`validity`) VALUES ('$reg[pincode]','$val','$userid','$reg[mobile]','$amount','".$arr['message']."','Debit','$date','$datetime','$ser[service_name]','$pro[provider_name]','$provider_id','".$arr['utr']."','".$arr['reportid']."','".$arr['orderid']."','".$arr['status_id']."','$optional1','$client_id','$expirydate','$validity')");
     
     
$postad = $db->query("INSERT INTO transactions (`type`,`pincode`,`transid`,`userid`,`merchant`,`merchantname`,`logo`,`paidby`,`customerno`,`amount`,`message`,`status`,`date`,`datetime`,`bill_status`,`sound_notification`,`service`,`provider`,`utr`,`reportid`,`orderid`,`status_id`,`number`,`client_id`,`provider_id`) VALUES ('Services','$reg[pincode]','$val','$userid','0','$pro[provider_name]','$pro[icon]','$reg[name]','$reg[mobile]','$amount','".$arr['message']."','Debit','$date','$datetime','0','0','$ser[service_name]','$pro[provider_name]','".$arr['utr']."','".$arr['reportid']."','".$arr['orderid']."','".$arr['status_id']."','$optional1','$client_id','$provider_id')");
...
include "bill_trans_notification.php";
...
$sql = $db->query("UPDATE recharges SET message = '".$arr['message']."' ,status = 'Failed',utr = '".$arr['utr']."',reportid ='".$arr['reportid']."' ,orderid = '".$arr['orderid']."',status_id='".$arr['status_id']."',response = '$arr'  WHERE  guid = '".$rech_id."' ");
...
$result = 'Payment processing , Please wait';
}else{ 


$sql = $db->query("UPDATE recharges SET message = '".$arr['message']."' ,status = 'Failure',utr = '".$arr['utr']."',reportid ='".$arr['reportid']."' ,orderid = '".$arr['orderid']."',status_id='".$arr['status_id']."',response = '$arr'  WHERE  guid = '".$rech_id."' ");
