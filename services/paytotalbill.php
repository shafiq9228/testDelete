...
$postad = $db->query("INSERT INTO transactions (`type`,`pincode`,`transid`,`userid`,`merchant`,`merchantname`,`paidby`,`customerno`,`amount`,`message`,`status`,`date`,`datetime`,`payment_id`,`status_message`,`txn_doing_for`) VALUES ('Bill Payment','$reg[pincode]','$val','$userid','0','Dakshinpay','$reg[name]','$reg[mobile]','$amount','Amount settled to dakshinpay','Credit','$date','$datetime','$payment_id','Success','Credit')");
...
$newpayment = $db->query("INSERT INTO bills (`pincode`,`userid`,`custid`,`name`,`email`,`mobile`,`subtotal`,`interest`,`waiveoff`,`total`,`duedate`,`date`,`datetime`,`deviceid`,`regid`,`status`,`fromdate`,`todate`) VALUES ('$reg[pincode]','$reg[guid]','$reg[reg_num]','$reg[name]','$reg[email]','$reg[mobile]','$balance','0','0','$balance','$date','$date','$datetime','$reg[deviceid]','$reg[regid]','Paid','$fromdate','$todate')");
*/
