...
//$postad = $db->query("INSERT INTO transactions (`type`,`pincode`,`transid`,`userid`,`merchant`,`merchantname`,`paidby`,`customerno`,`amount`,`message`,`status`,`date`,`datetime`,`payment_id`) VALUES ('Bill Payment','$reg[pincode]','$val','$userid','0','Dakshinpay','$reg[name]','$reg[mobile]','$amount','Amount settled to dakshinpay','Credit','$date','$datetime','$payment_id')");

$postad = $db->query("UPDATE transactions SET status = 'Credit',payment_id = '$payment_id',message = 'Amount settled to dakshinpay',status_message = 'Success'  WHERE  userid = '$userid' and guid = '$billid' ");
...
$sql = $db->query("INSERT INTO customer_notif(`transid`,`userid`,`message`,`date`,`datetime`,`status`) VALUES ('$billid','$userid','$notif','$date','$datetime','1')");
